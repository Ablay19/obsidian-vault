...
func (o *Orchestrator) AddProvider(name string, provider AIProvider) {
	...
	o.logger.Info("Added AI provider", "name", name, "model", modelInfo.ModelName)
}
...
func (o *Orchestrator) tryProvider(ctx context.Context, providerName string, req *RequestModel) (*ResponseModel, error) {
	...
	o.logger.AIRequest(providerName, req.Model, len(req.UserPrompt), 0) // TODO: Get user ID
...
func (o *Orchestrator) convertResponse(resp *ResponseModel, start time.Time) (*GenerationResult, error) {
	...
	modelInfo := resp.ProviderInfo

	o.logger.AIResponse(modelInfo.ProviderName, modelInfo.ModelName, resp.Usage.OutputTokens, latency.Milliseconds(), 0) // TODO: Get user ID
	...
	return &GenerationResult{
		...
		Latency:      latency,
		FinishReason: "completed", // TODO: Add finish reason to ResponseModel
	}, nil
}

...
The AI Fallback Service tries providers in this order:
...
```bash
# Uses CLOUDFLARE_API_TOKEN for authentication
...
The system automatically falls back to the next provider if:
- API request fails (401, 403, 429, 500, 502, 503)
- Request timeout exceeds configured limit
- Rate limit exceeded
...
Providers are automatically degraded if:
- Error rate exceeds 10% threshold
- Consecutive failures > 3

### System Prompt for Manim Generation

All providers use this system prompt:
